...
public final class FileIn {
    ...
    private FileIn(String path) {
        InputStream in = FileIn.class.getClassLoader().getResourceAsStream(path);
        if (in == null) {
    ...
    public static FileIn newInstance(String path) {
        return new FileIn(path);
    }
    ...
    /**
     * Returns true if standard input is empty (except possibly for whitespace). Use this method to know whether the
     * next call to {@link #readString()}, {@link #readDouble()}, etc will succeed.
     *
    ...
    /**
     * Returns true if standard input has a next line. Use this method to know whether the next call to {@link
