...
public class StdDrawDemo {
	...
	/*
	 * Draws random copies of the image, clearing in between drawings.
	 */
	public static void drawRandom() {
		int waitTimeMilliseconds = 100;

		/*
		 * Stamp 100 additional pictures in random locations, each one coming slightly
		...
		while (count < 200) {
			...
			/*
			 * Reduce wait time for each thing drawn, but never wait less than 10
			 * milliseconds.
			 */
	...
	/**
	 * Stick a copy of the image in the dead center of the image, which is position
	 * (0, 0). Slowly zoom in on the image, then zoom back out (but faster than we
	 * zoomed in).
	 */
	public static void drawZoom() {
		/**
		 * Enables double buffering. An animation technique where all drawing takes
		 * place on the offscreen canvas. Only when you call show() does your drawing
		 * get copied from the offscreen canvas to the onscreen canvas, where it is
		 * displayed in the standard drawing window. You don't have to understand this
		 * for CS61B. Just know that if you don't call this function, any attempt at
		 * smooth animation will look bad and flickery (remove it and see what
		 * happens!).
		 */
